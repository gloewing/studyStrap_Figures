...
seed.type <- "y"
tune.metric <- 6
sub.samp.ind <- 1 # 1 if we subsample
output.filename <- paste0("Stacked PCR Diff Results 0 Truncate (8-17)", 
...
colnames(final.results) <- c("Mean", "Weighted", "Weighted Distance", "CV Distance", "CV Distance Squared", "Individual Feats", "Individual RMSE", "Weighted Bag RMSE")
...
stack.results <- foreach(y=1:num.elec, .combine=list, .multicombine=TRUE, .packages = c("pls", "nnls")) %dopar%{
  stack.fn <- function(y){ 
    ...
    # Derivative
    current.elec <- as.data.frame(cbind(current.elec[,1], t(diff(t(current.elec[,-1])))))
    colnames(current.elec) <- c("DA", paste0("V_",1:(ncol(current.elec) - 1)))
    ...
    pcr.fit <- pcr(DA ~., data = as.data.frame(current.elec[train.rows,]), ncomp = best.comp) # fit classifier
    elec.RMSE <- sqrt(mean((current.elec[-train.rows,]$DA - predict(pcr.fit, current.elec[-train.rows, -1], ncomp = best.comp))^2))

    mod_RF <- pcr(DA ~., data = as.data.frame(current.elec), best.comp = 20) #fit RF on full dataset
    ...
    for (x in 1:num.elec){
      if(x == y){ #if same electrode classifier and design matrix
        ...
        #import and process current elec data (wiggle function)
          ...
          #predict.elec <- as.data.frame(cbind(predict.elec[,1], point.finder(predict.elec[,-1], predict.elec[,-1])[[i]])) #process data
        
        
        # Derivative
        predict.elec <- as.data.frame(cbind(predict.elec[,1], t(diff(t(predict.elec[,-1])))))
        colnames(predict.elec) <- c("DA", paste0("V_",1:(ncol(predict.elec) - 1)))
        
        #add predictions and labels to stack.preds matrix
        ...
        #attach x -- the electrode associated with the labels in each row
        }
    }
    #colnames(stack.preds) <- c("DA", paste0("E",1:num.elec)) #name columns DA and then electrodes 1:num.elec
...
results <- foreach(y=1:num.elec, .combine=list, .multicombine=TRUE) %dopar%{
    ...
    best.comp <- comp.vec[y]
      ...
      set.seed(y)
      final.results <- matrix(NA, ncol = 8, nrow = num.elec)
      colnames(final.results) <- c("Weighted-2", "Weighted-5", "Weighted-10", "Weighted-50","Weighted-100", "Individual Feats", "Within Bag RMSE", "Weighted Bag RMSE")
      print(paste0("Electrode ", y))
      test.num <- y
      if (sub.samp.ind == 1 ){
          test.elec <- read.csv(paste0("sub_samp", samp.size)) # took out dataframe
          colmns <- (ncol(test.elec)-1000):ncol(test.elec) # columns of full including DA (outcome) and design matrix
          X.colmns <- (ncol(test.elec)-999):ncol(test.elec) # columns of full just the design matrix
          test.elec <- test.elec[test.elec$Electrode == test.num, colmns]
      }else{
      ...
      for (z in seq(1, num.elec)[-y]){
        ...
        if (sub.samp.ind == 1 ){
            ...
            colmns <- (ncol(current.elec)-1000):ncol(current.elec) # columns of full including DA (outcome) and design matrix
            X.colmns <- (ncol(current.elec)-999):ncol(current.elec) # columns of full just the design matrix
            current.elec <- current.elec[current.elec$Electrode == z, colmns]
        }else{
        ...
        ################################################################## 
        print("PCR")
        pcr.fit.current <- pcr(DA~., data=as.data.frame(current.elec), ncomp = best.comp)
      ...
      sim.vec5 <- vector(length = nrow(similarity.matrix))
        ...
        for (x in 1:nrow(similarity.matrix)){
            #sim.vec[x] <- t(similarity.matrix[x,] - test.similarity) %*% (similarity.matrix[x,] - test.similarity)
            diff <- similarity.matrix[x,] - test.similarity
            sim.vec[x] <- sum(diff^2)
            sim.vec2[x] <- sum((diff * c(rep(20,4), rep(1,4)))^2) 
        }
        
        #for loop not necessary
      sim.vec3 <- stacked[[2]][y,]
      sim.vec4 <- stacked[[3]][y,]
        # #each row corresponds to the electrode that was taken out to make stacking weights
      print("dim rf preds")
      print(dim(RF.preds))
      ...
      # euclidean distance between modes squared * 20
...
results.mat <- matrix(NA, ncol = 8, nrow = num.elec)
for(i in 1:num.elec){
  results.mat[i,] <- results[[i]]
}
colnames(results.mat) <- c("Mean", "Mode", "Wiggle and Stacking", "Stacked no int", "Stacked Int", "Wiggle and Stacking", "Within Elec", "Mode-20")
