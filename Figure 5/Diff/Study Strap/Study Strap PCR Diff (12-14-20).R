# Gabe Loewinger
# 8/29/18
# Testing out Study Strap with bag sizes of just 13
# Allow repeats of how bag is made up
...
bag.size <- 75 #num.elec - 1 # number of random selection with replacement samples to put into a bag 
...
output.filename <- paste0("Study Strap PCR Derivative Opt Res_seed_", 
...
colnames(final.results) <- c("Mean", "Weighted", "Weighted Distance", "CV Distance", 
                             "Volt Dist", "Within Bag Feats", "Within Bag RMSE", 
                             "Weighted Bag RMSE", "Within Strap Feats", "Withinstrap test error", 
                             "Indiv Strap vs Test Elec Error", "OOB test error", "OOB Weighted", "Stacking")
...
results <- foreach(y=1:samps, .combine=list, .multicombine=TRUE) %dopar%{
  ...
  return.results <- function(y){
    ...
    for (z in 1:straps){
      ...
      # process dataset and add to similarity matrix
      ...
      pcr.fit <- pcr(DA ~., data = as.data.frame(cbind(DA = full$DA[indx], t(diff(t(full[indx, X.colmns]))))), 
      ...
      # stacking
      stackMat[,r] <- predict(pcr.fit, t(diff(t(full[stackIndx,X.colmns]))), ncomp = best.comp) # make predictions for stacking
      ...
      ############################
      # print(oob.error)      strap.results[z, 4] <- mean(oob.error) # mean OOB test error
