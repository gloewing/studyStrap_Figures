...
straps <- 200 #study replicates to include: HERE IS MAX STUDY REPLICATES
converge.lim <- 300000
...
rpts <- 10
tune.metric <- 0 # which OOB tuning metric to use (use 1 - 4), 0 corresponds to no tuning and just using pre-specified best.comp

seedVec <- seq(10, 100, by = 10)
set.seed(100)
seed.type <- paste0("y+", seedVec[seedNum])
output.filename <- paste0("SS PCR AR LR Derivative Opt Repeated Res_seed_", 
                          seed.type, "_samp_ ", samps, "_ncomp_", best.comp, "_LR_", eta, 
                              "_strap_", straps, "_conv_", converge.lim, "_tuneMet_", tune.metric, 
                                  "_tuneFrac_", tune.fraction, "_seqLen_", length(tune.grid), "_smpSz_",
                                        samp.size, "_bgSz_", bag.size, "_Rpts_", rpts)
...
final.results <- matrix(NA, ncol = 16, nrow = samps)
colnames(final.results) <- c("Mean", "Weighted", "Weighted Distance", "OOB Error All Rows", "OOB Elecs Merged", 
                             ...
                             "OOB Weighted", "In bag elecs-OOB rows", "Total Straps", "Stacking")
...
results <- foreach(y=1:samps, .combine=list, .multicombine=TRUE) %dopar%{
  ...
  return.results <- function(y){
    ...
    pcr.fit <- pcr(DA ~., data = test.elec, ncomp = best.comp, subset = train.rows,
                   model = FALSE, y = FALSE, x = FALSE) # took out dataframe
    ...
    paths <- 1 # number of repeated paths
    while(paths <= rpts && z <= straps){
        print(paste0("electrode: ", y, "strap #: ", z))
        print(paste0("electrode: ", y, "paths #: ", paths))
      ...
      while (sim == 0 && repeat.break == 0){ #while loop for restrapping: breaks for either acceptance (sim) or accpetance counter limit (converge.lim)
          ...
          if (counter >= converge.lim){
              counter <- 0
              sim.metric <- 10000000 # arbitrarily long
              paths <- paths + 1
              print(paste0("electrode: ", y, "paths #: ", paths))
              repeat.break <- 1 
          }
      ...
      if (tune.metric > 0 && tune.metric < 5){
        ...
        for ( i in 1:nrow(MSE.mat) ){
            ...
            #colnames(tune.samp) <- c("DA", paste0("V_",1:(ncol(tune.samp) - 1)))
            
            fit.tune <- pcr(DA ~., data = tune.samp, ncomp = tune.grid[i],
                            model = FALSE, y = FALSE, x = FALSE) # fit classifier on tune rows with current tuning parameter value
            ...
            if (tune.metric == 1){
              ...
              rm(oob.tune, oob.elec, oob.pred)
                ...
                #oob.elec <- full[is.element(full$Electrode, OOB), colmns] # make OOB electrode which has all rows except
      ...
      # put this into a vector to average for each foreach loop and put into final.results
      strap.results[r, 2] <- 1
      ...
      pcr.fit <- pcr(DA ~., data = ss, ncomp = best.comp,
                     model = FALSE, y = FALSE, x = FALSE) # fit classifier
    ...
    # stacking
    ##############
    stackMat <- stackMat[,1:row.total] # remove columns with NAs
