...
#' ##########################
#'
# Fit model with no target study. Use 1 Single-Study Learner (SSL): PCA Regression
...
sse <- function(formula = Y ~.,
  ...
  # # stack.standardize indicates whether stacking coefs are standardized
    ...
    straps <- length(original.studies) # study specific ensemble uses original studies as straps
    ...
    study.mat <- tibble::tibble(Study = unique(data$Study),
                        Study.Code = 1:length(unique(data$Study)) )
    data <- data %>%
    ...
    ss.obj <- list(models = model.list, data = c(), sim.mat <- c(),
                   ...
                   modelInfo = list(sampling = "studySpecificEnsemble", numStraps = straps, SSL = ssl.method,
                                    ssl.tuneGrid = c(), numPaths = NA,
                         convg.vec = c(), convgCritera = NA, meanSamp = NA, stack.type = "standard",
                         custFNs = customFNs, bagSize = 1),
    ...
    for (z in studies) {
        ...
        message(paste0("Study ", z))

        ########################
        ######## Study Rows
        ########################
        indx <- which(data$Study == z)
        ...
        if( !anyNA(target.study) ){
          ...
          if ( sim.mets == TRUE && length(customFNs) == 0){
            ...
            custom.matrix <- prop.table(abs(custom.matrix), 2)
