...
bagSzs <- c(3,5) #c(1:3, seq(4,18, by = 2), seq(20,100, by = 10), 200, 500, 1000)
bagSize.vec<- bagSzs
...
numBootItrs <- 3 # number of bootstrap replicates to estimate variance for similarity function
ssStrapsTune <- 3 # number of study straps for ss Tuning
ssStrapsTest <- 3 # number of study straps for Test SS
ssStrapsBagSzTest <- 3 # number of study straps for Test SS Bag Size

tune.grid <- c(0.001, 0.1) #sort( c(0.001, 0.01, 0.1, seq(0.0001, 5, length = 40) ) )
totalPaths <- 1 # paths for AR Test
convgLim <- 4 # convergence limit for AR
convgLimBagSize <- 4 #AR converge limit for AR bag size tune
convgLimBagSizeTest <- 4 #AR converge limit for AR bag size test
...
results <- foreach(iterNum = simItr, .combine = list, .multicombine = TRUE) %dopar%{
        ...
        #colnames(full) <- c("Study", "Y", paste0("V_", 1:(ncol(full) - 2) )  )
...
arBagSizeTest <- paste0("LASSO AR BagSize_Test_Sims5_")
ssBagSizeTest <- paste0("LASSO SS BagSize_Test_Sims5_")
