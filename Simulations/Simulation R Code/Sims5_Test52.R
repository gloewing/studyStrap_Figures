...
load.folder <- "/n/home12/gloewinger"
...
num.threads <- ceiling( totalSims / 3 ) # round up
...
results <-  foreach(iNum = simCntr, .combine = rbind, .multicombine = TRUE) %dopar%{
    
    print(paste0("START: ", iNum))
    setwd(save.folder)
    return.results <- function(itr){
        ...
        arBagSizeTest <- paste0("LASSO AR BagSize_Test_Sims5_", baseName)
        ssBagSizeTest <- paste0("LASSO SS BagSize_Test_Sims5_", baseName)
        
        set.seed(iterNum)
        #setwd(save.folder)
        
        # simulate data
        multiStudySim(sim.num = simNum,
                            iter = iterNum,
                                  sampSize = 400,
        ...
        # load data
        full <- read.csv(filename)# remove row names
        ...
        write.csv(full, filename, row.names = FALSE) # rewrite file without superfluous studies
        
        # test study
        test.elec <- as.data.frame(full[full$Study == test_study, -c(1,2)]) # set test electrode
        full <- as.data.frame(full[is.element(full$Study, trainStudies), -1]) # include only training studies in full, remove rownames
        ...
        #iterate through bag sizes
            ...
            for(bagSize in bagSzs){  
                ...
                for(y in trainStudies){
                    ...
                    full <- as.data.frame(read.csv(filename))
                    test.elec <- as.data.frame(full[full$Study == y, -c(1,2)]) # set test electrode
                    full <- as.data.frame(full[is.element(full$Study, original.Train), -1]) # include only training studies in full 
                    ...
                    for(study in 1:length(original.Train)){
                        indxList[[study]] <- which(full$Study != original.Train[study]) # indices of studies to train on
        ...
        if(!file.exists(ssTuneName)){
            # if this is the first thread to write CSV, write it
            # make matrix
            # cycle through bagsizes
            ssBagSizeTune <- matrix(ncol = length(bagSzs), nrow = totalSims)
            ssBagSizeCPSTune <- matrix(ncol = length(bagSzs), nrow = totalSims)
            ssBagSizeStackTune <- matrix(ncol = length(bagSzs), nrow = totalSims)
            
            # fill in corresponding row with colMeans
            ssBagSizeTune[iterNum,] <- colMeans(ssTuneResults)
            ssBagSizeCPSTune[iterNum,] <- colMeans(ssTuneResultsCPS)
            ssBagSizeStackTune[iterNum,] <- colMeans(ssTuneResultsStack)
            
            # write files
            write.csv(ssBagSizeTune, ssTuneName, row.names = FALSE)
            write.csv(ssBagSizeCPSTune, paste0(ssTuneName, "_CPS"), row.names = FALSE)
            write.csv(ssBagSizeStackTune, paste0(ssTuneName, "_Stack"), row.names = FALSE)
            
        }else{
            # if already written, read file, edit it and rewrite it 
            # read files
            ssBagSizeTune <- read.csv(ssTuneName)
            ssBagSizeCPSTune <- read.csv(paste0(ssTuneName, "_CPS"))
            ssBagSizeStackTune <- read.csv(paste0(ssTuneName, "_Stack"))
        ...
        full <- read.csv(filename) # remove row names
        test.elec <- as.data.frame(full[full$Study == test_study, -c(1,2)]) # set test electrode
        full <- as.data.frame(full[is.element(full$Study, trainStudies), -1]) # include only training studies in full 
        ...
        # if file exists read csv, add the relevant row and rewrite
        if(file.exists(ssBSz)){
            # simple average
            ssBagSizeMat <- read.csv(paste0(ssBagSizeTest, "_", "Avg"))
            ssBagSizeMat[iterNum,] <- ssBagSize[iterNum,]
            write.csv(ssBagSizeMat, paste0(ssBagSizeTest, "_", "Avg"), row.names = FALSE)
            rm(ssBagSizeMat)
            
            # CPS
            ssBagSizeMat <- read.csv( paste0(ssBagSizeTest, "_", "CPS") )
            ssBagSizeMat[iterNum,] <- ssBagSizeCPS[iterNum,]
            write.csv(ssBagSizeMat, paste0(ssBagSizeTest, "_", "CPS"), row.names = FALSE)
            rm(ssBagSizeMat)
            
            # Stack
            ssBagSizeMat <- read.csv( paste0(ssBagSizeTest, "_", "Stack") )
            ssBagSizeMat[iterNum,] <- ssBagSizeStack[iterNum,]
            write.csv(ssBagSizeMat, paste0(ssBagSizeTest, "_", "Stack"), row.names = FALSE)
            rm(ssBagSizeMat)
            
        }else{
            #otherwise just create the matrix
        ...
        #iterate through bag sizes
            ...
            # if this is the first thread to tune then run it and write CSV. If already run, then read csv saved
        ...
        if(!file.exists(arTuneName)){
            ...
            # fill in corresponding row with colMeans
            arBagSizeTune[iterNum,] <- colMeans(arTuneResults)
            arBagSizeCPSTune[iterNum,] <- colMeans(arTuneResultsCPS)
            arBagSizeStackTune[iterNum,] <- colMeans(arTuneResultsStack)
            ...
            # if already written, read file, edit it and rewrite it 
            # read files
            arBagSizeTune <- read.csv(arTuneName)
            arBagSizeCPSTune <- read.csv(paste0(arTuneName, "_CPS"))
            arBagSizeStackTune <- read.csv(paste0(arTuneName, "_Stack"))
        ...
        print(paste0("Write Test AR Bag Size: ", iterNum))
        arBSz <- paste0(arBagSizeTest, "_", "Avg")
        # if file exists read csv, add the relevant row and rewrite
        if(file.exists(arBSz)){
            # simple average
            arBagSizeMat <- read.csv(arBSz)
            arBagSizeMat[iterNum,] <- arBagSize[iterNum,]
            write.csv(arBagSizeMat, arBSz, row.names = FALSE)
            rm(arBagSizeMat)
            
            # CPS
            print(paste0("Write Test AR Bag Size CPS: ", iterNum))
            arBagSizeMat <- read.csv( paste0(arBagSizeTest, "_", "CPS") )
            arBagSizeMat[iterNum,] <- arBagSizeCPS[iterNum,]
            write.csv(arBagSizeMat, paste0(arBagSizeTest, "_", "CPS"), row.names = FALSE)
            rm(arBagSizeMat)
            
            # Stack
            print(paste0("Write Test AR Bag Size Stack: ", iterNum))
            arBagSizeMat <- read.csv( paste0(arBagSizeTest, "_", "Stack") )
            arBagSizeMat[iterNum,] <- arBagSizeStack[iterNum,]
            write.csv(arBagSizeMat, paste0(arBagSizeTest, "_", "Stack"), row.names = FALSE)
            rm(arBagSizeMat)
            
        }else{
            #otherwise just create the matrix
            write.csv(arBagSize, arBSz, row.names = FALSE)
        ...
        print(paste0("Remove Dataset: ", simNum, "_IterNum: ", iterNum ) )
        filename <- paste0("Sim ", simNum, "_", iterNum, "_Combined")
        if( file.exists(filename) ){
            print(paste0("File Exists: ", simNum, "_IterNum: ", iterNum ) )
            file.remove( filename ) # delete data
        }else{
            print( paste0("Filename Does not exist to delete: ", filename) )
        }
...
print("dim results: ")
print(dim(results))
colnames(res.mat) <- c("mrg", "sse", "sse_stack", "sse_cps", "ss", "ss_cps", 
                       "ss_stack", "ar", "ar_stack", "ar_cps")
setwd(save.folder)
write.csv(results, paste0("LASSO_Sims5_Results_", simNum), row.names = FALSE)
