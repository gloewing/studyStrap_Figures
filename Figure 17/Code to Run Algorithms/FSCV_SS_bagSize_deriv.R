...
comp.vec <- c(23, 22, 49, 38, 48, 22, 22, 22, 23, 22, 22, 23, 49, 20, 22) # official vector; changed 5/12/19
...
# 
bagSize.vec <- bagSzs
ssBagSz <- ARbagSize <- ARBagSize <- 75
...
fName <- paste0("FSCV_deriv_150_straps_3_paths_convg10000_bagSize_2500")
######### CHANGE NUMBER OF THREADS BEFORE RUNNING REAL THING !!!!!!!!!!!!!!!!!!!!!!!
logfile <- paste0("outputFile_fscv_deriv.txt")
...
results <- foreach(iterNum = 1:totalSims, .combine = list, .multicombine = TRUE) %dopar%{
        ...
        # derivative
        colmns <- (ncol(full)-999):ncol(full) # columns of full including DA (outcome) and design matrix
        X.colmns <- (ncol(full)-998):ncol(full) # columns of full just the design matrix
        
        full[,X.colmns] <- t(diff(t(full[,colmns])))
        full <- full[,-7] # remove extra column left over from taking derivative
        colnames(full)[-seq(1,6)] <- paste0("V_", 1:999)
        ...
        test.elec <- full[full$Electrode == y, (ncol(full) - 999):ncol(full)] # set test electrode
        print("full matrix")
        ...
        # similarity measure - dont use wiggle function for derivative
        simFn <- function(x1, x2){
            return( 1 / sum(  ( colMeans( x1) - colMeans(x2) )^2  )  )
        }
        ...
        mrg.mod <- merged(formula = Y ~., data = full, 
                          model = FALSE,
                          ssl.method = list("pcr"),
...
### names
filename <- paste0("fscv_deriv_Combined")
baseName <- paste0("fscv_deriv_Combined")
fileNm <- paste0("fscv_deriv_Combined")

arTuneName <- paste0("fscv_deriv AR Tune_", baseName)
ssTuneName <- paste0("fscv_deriv SS Tune_", baseName)
arBagSizeTest <- paste0("fscv_deriv AR BagSize_Test_")
ssBagSizeTest <- paste0("fscv_deriv SS BagSize_Test_")
